        ...
        {isMaterialImporterOpen && (
          <div className="material-importer-overlay" role="dialog" aria-label="Ajouter du matériel">
            <div className="material-importer-panel">
              ...
              {activeItemTab === "studios" && <div className="material-resource-list">{studios.length ? studios.map((studio) => <div className="material-resource-row" key={studio.id}><div><strong>{studio.name}</strong><span>Plateau / espace de production</span></div><b>{studio.dailyRate || 1200} €/jour</b><button type="button" onClick={() => handleAddStudioLine()}><Plus size={15} /> Ajouter</button></div>) : <div className="material-resource-empty"><Building2 size={32} /><strong>Aucun studio enregistré</strong><span>Ajoutez d’abord un studio dans l’annuaire.</span></div>}</div>}
              {activeItemTab === "technicians" && <div className="material-resource-list">{technicians.length ? technicians.map((tech) => <div className="material-resource-row" key={tech.id}><div><strong>{tech.name}</strong><span>Équipe technique / régie</span></div><b>{tech.dailyRate || 450} €/jour</b><button type="button" onClick={() => handleAddCrewLine()}><Plus size={15} /> Ajouter</button></div>) : <div className="material-resource-empty"><Users size={32} /><strong>Aucun personnel enregistré</strong><span>Ajoutez d’abord un technicien dans l’équipe.</span></div>}</div>}
